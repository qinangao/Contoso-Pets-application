    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 0] != "ID #: ")
                {
                    string currentAge = ourAnimals[i, 5].Replace("Age: ", "");
                    while (!int.TryParse(currentAge, out petAge))
                    {
                        Console.WriteLine($"Enter an age for {ourAnimals[i, 0]}");
                        readResult = Console.ReadLine();
                        if (readResult != null) currentAge = readResult;
                    }
                    ourAnimals[i, 2] = "Age: " + currentAge;

                    string currentDec = ourAnimals[i, 4].Replace("Physical description: ", "");
                    while (currentDec == "" || currentDec == "tbd")
                    {
                        Console.WriteLine($"Enter a physical description for {ourAnimals[i, 0]} (size, color, breed, gender, weight, housebroken)");
                        readResult = Console.ReadLine();
                        if (readResult != null) currentDec = readResult.Trim();
                    }
                    ourAnimals[i, 4] = "Physical description: " + currentDec;
                }
            }
        ...
        case "4":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 0] != "ID #: ")
                {
                    string currentName = ourAnimals[i, 3].Replace("Nickname: ", "");
                    while (currentName == "" || currentName == "tbd")
                    {
                        Console.WriteLine($"Enter a nickname for {ourAnimals[i, 0]}");
                        readResult = Console.ReadLine();
                        if (readResult != null) currentName = readResult.Trim();
                    }
                    ourAnimals[i, 3] = "Nickname: " + currentName;

                    string currentPerDec = ourAnimals[i, 5].Replace("Personality: ", "");
                    while (currentPerDec == "" || currentPerDec == "tbd")
                    {
                        Console.WriteLine($"Enter a personality description for {ourAnimals[i, 0]} (likes or dislikes, tricks, energy level)");
                        readResult = Console.ReadLine();
                        if (readResult != null) currentPerDec = readResult.Trim();
                    }
                    ourAnimals[i, 5] = "Personality: " + currentPerDec;
                }
            }
